...
PODCAST_ITUNES_SUMMARY = (
    "Midnight Metal Monastery is a Christian Rock and Metal podcast dedicated to lifting high the name of "
    "Jesus Christ through powerful music, Scripture, and faith. We are the Warrior Monks of Christian Rock—slamming "
    ...
    try:
        ...
        for file_info in data.get("files", []):
            ...
            if filename.lower().endswith((".mp3", ".m4a", ".ogg", ".flac", ".wav")):
                # Attempt to get duration from metadata (some Archive files include 'length' or 'playtime')
                duration_seconds = None
                if "length" in file_info and isinstance(file_info["length"], (int, float)):
                    duration_seconds = int(file_info["length"])
                elif "playtime" in file_info:
                    # playtime sometimes like "123.45" (seconds) or "mm:ss"
                    try:
                        # try numeric seconds first
                        duration_seconds = int(float(file_info["playtime"]))
                    except:
                        # try mm:ss
                        parts = str(file_info["playtime"]).split(":")
                        if len(parts) == 2:
                            try:
